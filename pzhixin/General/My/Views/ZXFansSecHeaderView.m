...
@implementation ZXFansSecHeaderView
...
- (void)createSubviews {
    ...
    if (!_operateBtn) {
        ...
        [_operateBtn setTitle:@"邀请人" forState:UIControlStateNormal];
        [_operateBtn.titleLabel setFont:[UIFont systemFontOfSize:12.0]];
        [_operateBtn setTitleColor:COLOR_666666 forState:UIControlStateNormal];
        [_sortView addSubview:_operateBtn];
    ...
    if (!_fansBtn) {
        ...
        [_fansBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(self.sortView);
            make.bottom.mas_equalTo(self.sortView);
            make.right.mas_equalTo(self.operateBtn.mas_left).mas_offset(-20.0);
        }];
    }
}
...
- (void)handleTapFansHeaderViewBtnAction:(UIButton *)button {
    if (self.zxFansSecHeaderViewBtnClick) {
        self.zxFansSecHeaderViewBtnClick(button.tag);
    }
}
