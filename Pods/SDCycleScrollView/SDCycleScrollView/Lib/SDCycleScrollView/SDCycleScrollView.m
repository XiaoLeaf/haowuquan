...
- (void)scrollToIndex:(int)targetIndex
{
    if (targetIndex >= _totalItemsCount) {
        if (self.infiniteLoop) {
            ...
            if (self.itemResumeBlock) {
                self.itemResumeBlock(0);
            }
        }
        return;
    }
...
//            [cell.imageView sd_setImageWithURL:[NSURL URLWithString:imagePath] placeholderImage:[UIImage imageNamed:@"placeholder_img_small"]];

            BOOL isExist = [[SDImageCache sharedImageCache] diskImageDataExistsWithKey:[[SDWebImageManager sharedManager] cacheKeyForURL:[NSURL URLWithString:imagePath]]];
            [cell.imageView sd_setImageWithURL:[NSURL URLWithString:imagePath] placeholderImage:_placeholderImage options:0 progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
                if (isExist) {
                ...
                if (!isExist) {
                    ...
                    [cell.imageView.layer addAnimation:basicAnimation forKey:@"opacity"];
