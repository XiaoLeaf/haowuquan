...
@interface ZXCommunityStoreCell () <UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout> {
    NSArray *imgList;
}
...
@implementation ZXCommunityStoreCell
...
- (void)setCommunityInfo:(NSDictionary *)communityInfo {
    ...
    NSString *comment = [_communityInfo objectForKey:@"comment"];
    NSMutableAttributedString *commentStr = [[NSMutableAttributedString alloc] initWithString:comment];
    [commentStr addAttributes:[UtilsMacro contentAttributes] range:NSMakeRange(0, comment.length)];
    
    [_headImg sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",[_communityInfo objectForKey:@"icon"]]] placeholderImage:nil];
...
- (void)createSubViews {
    ...
    if (!_commentView) {
        ...
        [_commentView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(self.imgContainer.mas_bottom).offset(15.0);
            make.left.mas_equalTo(self->_mainView.mas_left).offset(60.0);
            make.right.mas_equalTo(self->_mainView.mas_right).offset(-10.0);
            make.height.mas_equalTo(100.0).priorityLow();
            make.bottom.mas_equalTo(self->_mainView.mas_bottom).offset(-15.0).priorityHigh();
        }];
    }
    ...
    if (!_commentLabel) {
        ...
        [_commentLabel setTextColor:HOME_TITLE_COLOR];
        ...
        [_commentLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.bottom.mas_equalTo(self->_commentView).offset(-40.0);
        }];
    }
...
- (void)shareCommunityInfo {
    if (self.delegate && [self.delegate respondsToSelector:@selector(communityStoreCellShareCommunityInfo)]) {
        [self.delegate communityStoreCellShareCommunityInfo];
    }
}

- (void)copyCommunityComment {
    [UtilsMacro generalPasteboardCopy:_commentLabel.text];
    [ZXProgressHUD loadSucceedWithMsg:@"复制成功"];
}

- (void)checkGoodsDetail {
    if (self.zxCommunityStoreCellCheckDetail) {
        self.zxCommunityStoreCellCheckDetail();
    }
}
