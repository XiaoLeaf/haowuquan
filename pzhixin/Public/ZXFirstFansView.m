...
@implementation ZXFirstFansView
...
- (void)setUserInfo:(NSDictionary *)userInfo {
    ...
    if ([UtilsMacro whetherIsEmptyWithObject:[_userInfo valueForKey:@"nickname"]]) {
        [_nameLab setText:[NSString stringWithFormat:@"昵称:--"]];
    } else {
        [_nameLab setText:[NSString stringWithFormat:@"昵称:%@",[_userInfo valueForKey:@"nickname"]]];
    }
    if ([UtilsMacro whetherIsEmptyWithObject:[_userInfo valueForKey:@"wx"]]) {
        [_wxLab setText:[NSString stringWithFormat:@"微信号:--"]];
    } else {
        [_wxLab setText:[NSString stringWithFormat:@"微信号:%@",[_userInfo valueForKey:@"wx"]]];
    }
...
//        [_userHead setBackgroundColor:THEME_COLOR];
    ...
    if (!_nameLab) {
        ...
        [_nameLab setFont:[UIFont systemFontOfSize:12.0 weight:UIFontWeightMedium]];
        [_nameLab setTextColor:HOME_TITLE_COLOR];
        [_nameLab setText:@"昵称：4565131526"];
    ...
    if (!_wxLab) {
        ...
        [_wxLab mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.right.mas_equalTo(0.0);
            make.height.mas_equalTo(12.0);
            make.top.mas_equalTo(self.nameLab.mas_bottom).mas_offset(15.0);
        }];
    }
    ...
    if (!_phoneLab) {
        ...
        [_phoneLab mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.right.mas_equalTo(0.0);
            make.height.mas_equalTo(12.0);
            make.top.mas_equalTo(self.wxLab.mas_bottom).mas_offset(15.0);
        }];
    }
    ...
    if (!_bindBtn) {
        ...
        [_bindBtn setTitle:@"复制微信号" forState:UIControlStateNormal];
...
- (void)handleTapInviteViewBtnAction:(UIButton *)btn {
    if (self.zxFirstFansViewBtnClick) {
        self.zxFirstFansViewBtnClick(btn.tag);
    }
}
