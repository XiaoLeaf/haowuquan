...
-(UIView *)initAddressViewWithHeiht:(CGFloat)height andSafeHeight:(CGFloat)safeHeight {
    _totalHeight = height;
    ...
    UILabel * titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(20, 10, screen_width - 80, 30)];
    titleLabel.text = _title;
//    titleLabel.textAlignment = NSTextAlignmentCenter;
    titleLabel.textColor = HOME_TITLE_COLOR;
    ...
    cancelBtn.tag = 1;
    [cancelBtn setImage:[UIImage imageNamed:@"ic_my_close"] forState:UIControlStateNormal];
...
-(void)addAnimateWithHeight:(CGFloat)height {
    self.hidden = NO;
    [UIView animateWithDuration:0.2 animations:^{
        self.addAddressView.frame = CGRectMake(0, self.totalHeight - self.defaultHeight, screen_width, self.defaultHeight);
    }];
}
...
-(void)setupAllTitle:(NSInteger)selectId{
    ...
    _lineLabel.backgroundColor = THEME_COLOR;
    ...
    for (int i = 0; i < self.titleMarr.count ; i++) {
        ...
        titleBtn.tag = i;
        [titleBtn setTitleColor:HOME_TITLE_COLOR forState:UIControlStateNormal];
         [titleBtn setTitleColor:THEME_COLOR forState:UIControlStateSelected];
...
-(void)titleBtnClick:(UIButton *)titleBtn{
    ...
    self.lineLabel.frame = CGRectMake(CGRectGetMinX(titleBtn.frame), self.titleScrollViewH - 1.0,titleBtn.frame.size.width, 1.0);
...
- (void)getAddressMessageDataAddressID:(NSInteger)addressID  provinceIdOrCityId: (NSString *)provinceIdOrCityId{
    if (addressID == 1) {
        ...
        [self caseCountyArr:self.cityList withSelectedID:provinceIdOrCityId];
    }
    else if(addressID == 4){
        [self caseTownArr:self.countryList withSelectedID:provinceIdOrCityId];
    }
...
-(void)caseCityArr:(NSArray *)cityArr withSelectedID:(NSString *)selectedID{
    ...
    for (NSDictionary *dic in cityArr) {
        if ([[NSString stringWithFormat:@"%@",dic[@"id"]] isEqualToString:selectedID]) {
            self.cityList = [[NSArray alloc] initWithArray:dic[@"c"]];
            for (NSDictionary *subDic in dic[@"c"]) {
                NSDictionary * dic1 = @{@"id":subDic[@"id"],
                                        @"city_name":subDic[@"n"]};
...
-(void)caseCountyArr:(NSArray *)countyArr withSelectedID:(NSString *)selectedID{
    ...
    for (NSDictionary *dic in countyArr) {
        if ([[NSString stringWithFormat:@"%@",dic[@"id"]] isEqualToString:selectedID]) {
            self.countryList = [[NSArray alloc] initWithArray:dic[@"c"]];
            for (NSDictionary *subDict in dic[@"c"]) {
                NSDictionary * dic1 = @{@"id":subDict[@"id"],
                                        @"county_name":subDict[@"n"]};
